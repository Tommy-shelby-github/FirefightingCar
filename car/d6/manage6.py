...
from donkeycar.parts.model.yolo3 import YunTAB
from donkeycar.parts.model.uploadit import SFTP1
from donkeycar.parts.model.downloadit import SFTP2
from donkeycar.parts.launch import AiLaunch
from donkeycar.parts.water import Water
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    #Initialize car         初始化车辆
    V = dk.vehicle.Vehicle()

    #Initialize logging before anything else to allow console logging
    if cfg.HAVE_CONSOLE_LOGGING:#----------------True
    ...
    uploadimg=SFTP1()
    downloadtxt=SFTP2()
    V.add(uploadimg,inputs=['cam/image_array'],threaded=True)
    V.add(downloadtxt,outputs=['pilot/angle_down', 'pilot/angle_up'],threaded=True)
...
#-------------------------------------------------------------------------------------------------------------
    class ThrottleByFire:
        def run(self,angle_up,throttle):
            if((angle_up>=0.2)&(angle_up<=0.9)):
                throttle=0.1
                state=True
            else:
                state=False
    ...
    #根据云台上扬角度控制车是否停止
    V.add(ThrottleByFire(),inputs=['pilot/angle_up','throttle'],outputs=['throttle','state'])
    V.add(Water(),inputs=['state'])
#-------------------------------------------------------------------------------------------------------------
    ...
    if cfg.DONKEY_GYM or cfg.DRIVE_TRAIN_TYPE == "MOCK":
        ...
        steering_down_controller = PCA9685(cfg.STEERING_DOWN_CHANNEL, cfg.PCA9685_I2C_ADDR, busnum=cfg.PCA9685_I2C_BUSNUM)
        steering_down = PWMSteering(controller=steering_down_controller,
                                        left_pulse=cfg.STEERING_DOWN_LEFT_PWM,
                                        right_pulse=cfg.STEERING_DOWN_RIGHT_PWM)
        steering_up_controller = PCA9685(cfg.STEERING_UP_CHANNEL, cfg.PCA9685_I2C_ADDR, busnum=cfg.PCA9685_I2C_BUSNUM)
        steering_up = PWMSteering(controller=steering_up_controller,
                                        left_pulse=cfg.STEERING_UP_LEFT_PWM,
                                        right_pulse=cfg.STEERING_UP_RIGHT_PWM)
        ...
        V.add(steering_down, inputs=['pilot/angle_down'], threaded=True)
        V.add(steering_up, inputs=['pilot/angle_up'], threaded=True)
